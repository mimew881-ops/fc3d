...
function updateCountdown() {
  ...
  if (!currentContext.matched || !currentItem?.date) {
    const [targetHour, targetMinute] = sharedStore.normalizeTimeText(pageState.config.drawTime, "22:15").split(":").map(Number);
  ...
  if (drawState.isComplete) {
    countdownNode.textContent = "已開獎";
    return;
  }

  const drawTime = sharedStore.normalizeTimeText(pageState.config.drawTime, "22:15");
  const target = new Date(`${currentItem.date}T${drawTime}:00`);
