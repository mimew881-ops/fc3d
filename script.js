...
const homeNodes = {
  ...
  trendMiniRows: document.querySelector("#homeTrendMiniRows")
};
...
function getDisplayContext(now = new Date()) {
  const currentIssue = String(pageState.config.currentIssue || "").trim();
  const currentItem = currentIssue ? pageState.results.find((item) => item.issue === currentIssue) : null;
  const previousItem = pageState.results.find((item) => item.issue !== currentIssue) || null;
  ...
  if (currentItem && currentItem.date) {
    const drawStart = new Date(`${currentItem.date}T${drawTime}:00`);
  ...
  const displayItem = switchedToCurrent && currentItem ? currentItem : (previousItem || currentItem);

  return {
    currentIssue: currentIssue || currentItem?.issue || previousItem?.issue || "",
    currentItem: currentItem || null,
    previousItem: previousItem || null,
    displayItem: displayItem || null,
    switchedToCurrent: Boolean(switchedToCurrent && currentItem),
    drawTime
  };
}

function isIssueClosed(item, now = new Date()) {
  const currentIssue = String(pageState.config.currentIssue || "").trim();
  if (item.issue !== currentIssue) {
    return true;
  }
  const drawTime = sharedStore.normalizeTimeText(pageState.config.drawTime, "22:15");
  ...
  return now.getTime() - drawStart.getTime() >= 20000;
}
...
function renderHomePage() {
  ...
  const now = new Date();
  const ctx = getDisplayContext(now);
  const displayItem = ctx.displayItem;
  if (!displayItem) {
    return;
  }

  const drawTime = ctx.drawTime;

  let drawState;
  let visible;
  if (ctx.switchedToCurrent) {
    drawState = getTrackedDrawState(displayItem, now);
    visible = getVisibleResult(displayItem, now);
  } else {
    drawState = { isPending: true, isRevealing: false, isComplete: false };
    visible = getStoredResult(displayItem);
  }

  if (homeNodes.topDrawTime) homeNodes.topDrawTime.textContent = drawTime;
  if (homeNodes.currentIssueHeader) homeNodes.currentIssueHeader.textContent = ctx.currentIssue || "--";
  if (homeNodes.infoUpdateTime) homeNodes.infoUpdateTime.textContent = drawTime;
  if (homeNodes.latestPanelIssue) homeNodes.latestPanelIssue.textContent = `第${displayItem.issue}期`;
  if (homeNodes.latestDate) homeNodes.latestDate.textContent = displayItem.date || "--";
  ...
  if (homeNodes.latestTest) homeNodes.latestTest.textContent = displayItem.test || "--";
  ...
  if (homeNodes.drawNotice) {
    if (!ctx.currentItem) {
      homeNodes.drawNotice.textContent = `台灣3D福彩第${ctx.currentIssue}期已設為當前期，請先到後台保存這一期的開獎日期與號碼。`;
    } else if (ctx.switchedToCurrent && drawState.isRevealing) {
      homeNodes.drawNotice.textContent = `台灣3D福彩第${ctx.currentIssue}期正在開獎，已開出第 ${drawState.openedCount} 個號碼。`;
    } else if (ctx.switchedToCurrent && drawState.isComplete) {
      homeNodes.drawNotice.textContent = `台灣3D福彩第${ctx.currentIssue}期已完成開獎，試機號、和值、跨度與型態已同步更新。`;
    } else {
      homeNodes.drawNotice.textContent = `台灣3D福彩第${ctx.currentIssue}期將於 ${ctx.currentItem?.date || ""} ${drawTime} 開獎，開獎後系統將同步更新號碼、試機號與歷史記錄。`;
    }
  }

  if (homeNodes.latestArticleText) {
    const done = ctx.switchedToCurrent && drawState.isComplete;
    homeNodes.latestArticleText.textContent = `台灣3D福彩第${ctx.currentIssue}期${done ? "已完成開獎並同步更新試機號、和值與型態說明。" : "即將開獎，開獎後將同步更新試機號、和值與型態說明。"}`;
  }
  if (homeNodes.latestArticleDate) {
    homeNodes.latestArticleDate.textContent = formatShortDate(displayItem.date);
  }

  if (homeNodes.homeHistoryBody) {
    const closedResults = pageState.results.filter((item) => isIssueClosed(item, now));
    homeNodes.homeHistoryBody.innerHTML = closedResults.slice(0, 6).map((item) => {
      const rowVisible = getStoredResult(item);
      return `
        <tr>
          <td>${item.issue}</td>
...
function renderTrendMini(now = new Date()) {
  if (!homeNodes.trendMiniRows) {
    return;
  }
  const closedResults = pageState.results.filter((item) => isIssueClosed(item, now));
  homeNodes.trendMiniRows.innerHTML = closedResults.slice(0, 6).map((item, index) => {
    const seq = String(item.issue).slice(4).padStart(4, "0");
    return `
      <div class="trend-mini-row${index === 0 ? " active" : ""}">
        <span>${seq}</span>
        <b>${item.numbers[0]}</b>
        <b>${item.numbers[1]}</b>
        <b>${item.numbers[2]}</b>
...
function getFilteredHistoryResults(now = new Date()) {
  const keyword = String(pageState.historyKeyword || "").trim();
  const closed = pageState.results.filter((item) => isIssueClosed(item, now));
  if (!keyword) {
    return closed;
  }
  return closed.filter((item) => item.issue.includes(keyword) || item.date.includes(keyword));
}
...
function renderHistoryPage() {
  ...
  const now = new Date();
  const filtered = getFilteredHistoryResults(now);
