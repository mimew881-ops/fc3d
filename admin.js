...
function saveConfig() {
  ...
  state.config = {
    currentIssue: elements.currentIssueInput.value.trim() || state.config.currentIssue || state.results[0]?.issue || "",
    drawTime
  };
  persistConfig();
  renderConfigForm();
  renderStats();
  appendLog(`配置已保存，当前期号 ${state.config.currentIssue || "--"}，开奖时间 ${drawTime}`);
}
...
function saveResult() {
  ...
  persistResults();
  state.config.currentIssue = issue;
  persistConfig();
  renderConfigForm();
  renderStats();
  renderResultTable();
  appendLog(`第 ${issue} 期已保存，号码 ${numbers.join("")}，形态 ${result.shape}`);
  clearForm();
}
